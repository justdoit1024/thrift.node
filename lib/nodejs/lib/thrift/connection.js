...
var Connection = exports.Connection = function(stream, options) {
  ...
  this.respondWithConnectionError = function(data, err) {
    this.transport.receiver(decodeConnectionErrorCallback.bind(this))(data);
  };
  ...
  this.connection.addListener("error", function(err) {
    ...
    //console.log("connection addListener error" , err);
...
Connection.prototype.write = function(data) {
  if (!this.connected) {
    this.respondWithConnectionError(data, new Error('connect closed.'));
    return;
  }
...
Connection.prototype.connection_gone = function () {
  ...
  console.debug("Retry connection in " + this.retry_delay + " ms");

  if (this.max_attempts && this.attempts >= this.max_attempts) {
    this.retry_timer = null;
    console.log("thrift: Couldn't get thrift connection after " + this.max_attempts + " attempts.");
    self.emit("close");
    return;
  }
